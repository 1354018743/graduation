...
public class StudentApplyMessageDao {
	...
	public List<ApplyProgress> queryApplyMessage(int lid,String sno,String sname) {
		Connection connection=DBUtil.getConnection();
		QueryRunner queryRunner=new QueryRunner();
		int rank=getRank(lid)-1;
		try {
			System.out.println(sname);
			List<ApplyProgress> applyProgress=queryRunner.query(connection, "select * from applyprogress where sname like '%"+sname+"%' and sno like '%"+sno+"%'  and  progress="+rank+"", new BeanListHandler<>(ApplyProgress.class));
			System.out.println(applyProgress.size());
	...
	//分页查询申请信息
	 public  List<ApplyProgress>  queryPage(int page,int pageSize,int lid,String sno,String sname){
		 ...
		 Connection con=DBUtil.getConnection();
		  ...
		  int rank=getRank(lid)-1;
		try {
			List<ApplyProgress> list=  qr.query(con, "select * from applyprogress where   sname like '%"+sname+"%' and sno like '%"+sno+"%' and  progress="+rank+" limit "+(page-1)*pageSize+','+pageSize,new BeanListHandler<>(ApplyProgress.class));
